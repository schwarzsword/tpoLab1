...
class TestArccos {
    ...
    @ParameterizedTest
    @ValueSource(doubles = {0.5, -0.5, 0, 0.95, -0.75, 0.99, -0.99})
    void calcDefinedAcos(double x) {
        Assertions.assertEquals(MyAcos.calcAcosIn(x).doubleValue(), Math.acos(x), 0.001);
    }

    @ParameterizedTest
    @ValueSource(doubles = {1, -1})
    void calcDefinedAcosEndVal(double x) {
